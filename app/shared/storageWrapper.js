...
class StorageWrapper {
        ...
        if (data) {
          ...
          if (jsonData.keys && Array.isArray(jsonData.keys)) {
            const transformedKeys = jsonData.keys.map(key => ({
              ...key,
              id: uuid()  // Adding a random UUID to each key
            }));
          ...
          resolve(null);  // Change this to resolve("{}") if you want to always return an empty JSON object when there's no data
        }
      });
    });
  }
  ...
  static async initShortkeys() {
    ...
    if (!shortkeysData) {
      StorageWrapper.shortkeys = DEFAULT_SHORTKEYS_JSON;
